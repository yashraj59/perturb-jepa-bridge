...
| --- | ---: | ---: | --- |
| F008 train-only heldout-set geometry audit | 0.891 | 0.700 | Run F008_TRAIN_ONLY_HELDOUT_SET_GEOMETRY_AUDIT. |
| C004 representation support repair target specification | 0.780 | 0.650 | Defer until train-only split geometry quantifies whether the benchmark is under-supported. |
| G001 fresh synthetic seed latent-cache audit | 0.810 | 0.650 | Defer until the cheaper current-cache split geometry audit runs. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F008 train-only heldout-set geometry audit`.

## Next Experiment Command Or Target
`Run F008_TRAIN_ONLY_HELDOUT_SET_GEOMETRY_AUDIT.`
