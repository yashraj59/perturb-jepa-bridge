...
| --- | ---: | ---: | --- |
| F030 delta-direction transition repair | 0.863 | 0.760 | Add explicit delta-direction and source-improvement losses to the small real JEPA transition predictor. |
| Train F029 longer | 0.631 | 0.220 | Reject because F029 already has low train transition loss but fails the held-out delta-direction gate. |
| Return to representation-only repair | 0.680 | 0.360 | Reject because F029 cross-modal retrieval and ridge floor are non-collapsed; the immediate failure is transition direction. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F030 delta-direction transition repair`.

## Next Experiment Command Or Target
`Add explicit delta-direction and source-improvement losses to the small real JEPA transition predictor.`
