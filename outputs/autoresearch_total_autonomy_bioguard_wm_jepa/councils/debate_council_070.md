...
| --- | ---: | ---: | --- |
| F041 constraint-ablation diagnostic | 0.857 | 0.580 | Use existing train/eval scale grids to locate which train-only safety constraints reject oracle-safe nonzero residuals. |
| Train a larger retrieval-aware head | 0.663 | 0.440 | Defer because F040 already changed the loss and still selected zero; identify the selection bottleneck before more training. |
| Adopt F038 oracle scales | 0.654 | 0.100 | Reject because held-out oracle scale labels cannot be used for deployable calibration or model promotion. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F041 constraint-ablation diagnostic`.

## Next Experiment Command Or Target
`Use existing train/eval scale grids to locate which train-only safety constraints reject oracle-safe nonzero residuals.`
