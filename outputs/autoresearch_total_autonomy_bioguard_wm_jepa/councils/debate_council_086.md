...
| --- | ---: | ---: | --- |
| F057 six-seed row-wise synthesis | 0.886 | 0.360 | Aggregate F055/F056 into a read-only six-seed synthesis before drafting a Tier 2 design. |
| Promote row-wise abstention immediately | 0.637 | 0.200 | Reject because only Tier 3 can promote and this is still synthetic diagnostic evidence. |
| Skip synthesis and train a larger predictor | 0.683 | 0.340 | Defer until the current evidence is summarized and converted into exact next gates. |

## Monitor Decision
`COUNCIL_EXECUTE_METRIC_OR_INTERNAL_AUDIT`

## Exact Next Amendment
Execute `F057 six-seed row-wise synthesis`.

## Next Experiment Command Or Target
`Aggregate F055/F056 into a read-only six-seed synthesis before drafting a Tier 2 design.`
