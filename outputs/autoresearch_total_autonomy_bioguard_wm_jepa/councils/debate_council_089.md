...
| --- | ---: | ---: | --- |
| F060 floor-direction-projected row-wise residual | 0.809 | 0.580 | Project residuals onto the positive floor-delta direction before train-only row-wise abstention to reduce transition-source cosine instability. |
| Tighten train transition gate only | 0.774 | 0.340 | Defer because F059 found train transition gain anti-correlates with held-out transition gain. |
| Abandon residuals and return to representation warmstart | 0.677 | 0.420 | Defer until a cheap geometry-constrained residual probe tests whether transition instability can be removed. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F060 floor-direction-projected row-wise residual`.

## Next Experiment Command Or Target
`Project residuals onto the positive floor-delta direction before train-only row-wise abstention to reduce transition-source cosine instability.`
