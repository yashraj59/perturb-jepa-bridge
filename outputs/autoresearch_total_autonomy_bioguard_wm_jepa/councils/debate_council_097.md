...
| --- | ---: | ---: | --- |
| F068 data-contract calibration redirect | 0.911 | 0.700 | Compare extrapolative, random, and stratified perturbation holdout contracts before any further model training. |
| Reopen residual selectors with stricter LCB | 0.677 | 0.240 | Reject because F067 closed this family under the current data contract. |
| Train larger representation immediately | 0.631 | 0.240 | Reject until the data contract tells us whether train calibration can predict heldout scoring. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F068 data-contract calibration redirect`.

## Next Experiment Command Or Target
`Compare extrapolative, random, and stratified perturbation holdout contracts before any further model training.`
