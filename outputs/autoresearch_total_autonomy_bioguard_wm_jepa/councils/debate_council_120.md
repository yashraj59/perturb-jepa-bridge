...
| --- | ---: | ---: | --- |
| F091 scGeneScope feature storage/backed-IO preflight | 0.907 | 0.560 | Use only saved remote metadata to estimate smallest paired feature payload, disk/RAM requirements, and backed H5AD dependencies before any payload download. |
| Download smallest paired feature files immediately | 0.640 | 0.340 | Reject until storage, RAM, backed-IO, and low-compute payload gates are written and logged. |
| Return to synthetic architecture search | 0.674 | 0.320 | Reject because F086-F090 identify external validation readiness as the current bottleneck. |

## Monitor Decision
`COUNCIL_EXECUTE`

## Exact Next Amendment
Execute `F091 scGeneScope feature storage/backed-IO preflight`.

## Next Experiment Command Or Target
`Use only saved remote metadata to estimate smallest paired feature payload, disk/RAM requirements, and backed H5AD dependencies before any payload download.`
