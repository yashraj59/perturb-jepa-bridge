...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a saved BioTech-JEPA checkpoint.")
    ...
    batches = _eval_batches(args, device=device)
    metrics = evaluate_biotech_batches(model, batches, device=device)
...
def _eval_batches(args: argparse.Namespace, *, device: torch.device):
